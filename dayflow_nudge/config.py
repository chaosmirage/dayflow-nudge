...
command-line flags. Exactly three variables are consulted, on every cycle,
...
def load_config(environ: Mapping[str, str] | None = None) -> Config:
    """Read the three DFN_* variables into a typed Config.
    ...
    wins, so the loop can never spin on a zero-second tick.
    """
